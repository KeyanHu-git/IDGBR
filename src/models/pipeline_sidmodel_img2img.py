...
from src.models.UnetModel import (
    UNet2DConditionEncodeModel,
    UNet2DConditionNewModel,
)
...
class StableDiffusionSIDModelImg2ImgPipeline(
    DiffusionPipeline, TextualInversionLoaderMixin, LoraLoaderMixin, FromSingleFileMixin
):
    r"""
    Pipeline for image-to-image generation using Stable Diffusion with SIDModel guidance.
    ...
    def __init__(
        ...
        unet_ori: UNet2DConditionEncodeModel,
        unet_gen: UNet2DConditionNewModel,
    ...
    def __call__(
        ...
        **kwargs,
        ...
        self.interact_direction = interact_direction
        ...
        # sidmodel = self.sidmodel._orig_mod if is_compiled_module(self.sidmodel) else self.sidmodel
        sidmodel = SIDModel(
            unet_gen=self.unet_gen,
            interact_net=self.interact_net,
            label_embed_net=self.label_embed_net,
            unet_ori=self.unet_ori,
        )
        ...
        global_pool_conditions = (
            # sidmodel.unet_copy.config.global_pool_conditions
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                # TODO expand the latents if we are doing classifier free guidance
                latent_model_input = self.scheduler.scale_model_input(latents, t)
                
                if self.do_classifier_free_guidance:
                    latent_model_input_2 = torch.cat(
                        [latent_model_input, latent_model_input]
                    )

                noise_pred, representators_tilde = sidmodel.predict(
                    noise_latents=latent_model_input_2,
                    img_latents=cond_latents,
                    seg_latents=rough_lbl_latents,
                    timesteps=t,
        ...
        if hasattr(self, "final_offload_hook") and self.final_offload_hook is not None:
            self.unet_ori.to("cpu")
            self.unet_gen.to("cpu")
            self.interact_net.to("cpu")
            self.label_embed_net.to("cpu")
            torch.cuda.empty_cache()

        embed = self.vae.decode(
            latents / self.vae.config.scaling_factor,
            return_dict=False,
            generator=generator,
        )[0]

        embed = self.label_embed_net.decode(embed)
        seg_pred_logits = F.softmax(embed, dim=1)
        seg_image_pred = torch.argmax(seg_pred_logits, dim=1)

        has_nsfw_concept = None

        seg_images = [
            PIL.Image.fromarray(mask.astype(np.uint8))
            for mask in seg_image_pred.detach().cpu().numpy()
        ]
        if len(seg_images) == 1:
            seg_images = seg_images[0]

        # Offload all models
        self.maybe_free_model_hooks()

        if not return_dict:
            return (seg_images, has_nsfw_concept)

        return StableDiffusionPipelineOutput(
            images=seg_images, nsfw_content_detected=has_nsfw_concept
        )


if __name__ == "__main__":
    a = StableDiffusionSIDModelImg2ImgPipeline()
